...
# ── Step 6: Datadog Lambda layer already attached via CFN ─────
echo ""
echo "═══ [6/7] Datadog Lambda instrumentation... ═══"
echo "  Datadog-Python311 layer and DD_API_KEY injected via CloudFormation."
echo "  Lambdas ship fully instrumented — no manual steps required."
echo ""
...
REACT_APP_ORDER_API=${FRONTEND_URL}
REACT_APP_CART_API=${FRONTEND_URL}
REACT_APP_AUTH_API=${FRONTEND_URL}
REACT_APP_PAYMENT_API=${FRONTEND_URL}
...
echo "║  Orders API:  ${FRONTEND_URL}/api/products"
echo "║  Cart API:    ${FRONTEND_URL}/api/cart"
echo "║  Auth API:    ${FRONTEND_URL}/api/auth"
echo "║  Payment API: ${FRONTEND_URL}/api/payment"
...
echo "║  - Lambda:   Datadog-Python311 layer (via CFN)      ║"
